...
`survey.provenance_signals()` exists to report that, and
`test_no_note_carries_a_signal_of_who_wrote_it` asserts it, so that the day a vault does
grow such a key the assumption fails loudly instead of quietly continuing to hold.
...
exactly this, and `tests/test_vault_survey.py` exercises each rule.
